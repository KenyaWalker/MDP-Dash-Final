...
// Updated: Added delete endpoint for survey cleanup - Sept 24, 2025
...
// Delete survey response endpoint (for cleanup)
app.delete('/api/survey-responses/:id', (req, res) => {
    try {
        ...
        // Filter out the survey with the specified ID
        const filteredData = data.filter(survey => survey.id !== id);
        ...
        if (writeData(filteredData)) {
            console.log(`🗑️ Survey deleted: ID ${id}`);
            res.json({ 
