...
class MDPDashboard {
    ...
    setupAutoRefresh() {
        // Auto-refresh every 60 seconds for real-time updates (preserves current view)
        setInterval(async () => {
        ...
        }, 60 * 1000); // 60 seconds (doubled interval to be less disruptive)
    }
    ...
    populateRoleFilter() {
        ...
        // Filter out undefined/null values and get unique roles
        const roles = [...new Set(this.data.map(item => item.evaluatorRole).filter(role => role))].sort();
    ...
    applyFilters() {
        ...
        this.filteredData = this.data.filter(item => {
            ...
            if (this.filters.role && item.evaluatorRole !== this.filters.role) return false;
    ...
    getIndividualTableRows() {
        ...
        return sortedGroups.map(group => {
            ...
            const avgScores = {
                jobKnowledge: submissions.reduce((sum, s) => sum + s.scores['Job Knowledge'], 0) / submissions.length,
                qualityOfWork: submissions.reduce((sum, s) => sum + s.scores['Quality of Work'], 0) / submissions.length,
                communication: submissions.reduce((sum, s) => sum + s.scores['Communication Skills & Teamwork'], 0) / submissions.length,
                initiative: submissions.reduce((sum, s) => sum + s.scores['Initiative & Productivity'], 0) / submissions.length,
