...
class MDPDashboard {
    ...
    async init() {
        try {
            ...
            await this.loadData();
            console.log('Data loaded, setting up dashboard...');
    ...
    setupAutoRefresh() {
        // Auto-refresh disabled to prevent view interruptions during data analysis
        // Users can manually refresh using the refresh button when needed
        console.log('Auto-refresh disabled - manual refresh only');
        
        // Optional: Uncomment below to re-enable auto-refresh
        /*
        setInterval(async () => {
            ...
            try {
                ...
                // Restore filters and view after data load
                this.filters = currentFilters;
                this.currentView = currentView;
                
                this.applyFilters();
                this.updateChartsOnly(); // Only update charts, don't reset view
                this.updateLastUpdated();
                console.log('Dashboard auto-refresh completed (view preserved)');
            } catch (error) {
    ...
    async loadData() {
        try {
            ...
            this.data = rawData.map(item => {
                // Calculate scores if missing (for backward compatibility)
                let jobKnowledge = item.jobKnowledge;
                let qualityOfWork = item.qualityOfWork; 
                let communication = item.communication;
                let initiative = item.initiative;
                let overall = item.overall;
                
                if (!jobKnowledge && item.responses) {
                    // Calculate scores from responses if missing
                    const responses = item.responses;
                    jobKnowledge = ((responses.Q1 || 0) + (responses.Q2 || 0) + (responses.Q3 || 0) + (responses.Q4 || 0)) / 4;
                    qualityOfWork = ((responses.Q5 || 0) + (responses.Q6 || 0)) / 2;
                    communication = ((responses.Q7 || 0) + (responses.Q8 || 0)) / 2;
                    initiative = ((responses.Q9 || 0) + (responses.Q10 || 0)) / 2;
                    overall = (jobKnowledge + qualityOfWork + communication + initiative) / 4;
                }
                ...
                return {
                    ...
                    overall: overall || 0,
                    jobKnowledge: jobKnowledge || 0,
                    qualityOfWork: qualityOfWork || 0,
                    communication: communication || 0,
                    initiative: initiative || 0,
                    scores: {
                        'Job Knowledge': jobKnowledge || 0,
                        'Quality of Work': qualityOfWork || 0,
                        'Communication Skills & Teamwork': communication || 0,
                        'Initiative & Productivity': initiative || 0
                    }
                };
            });
            ...
            console.log('Sample transformed data:', this.data[0]);
    ...
    populateManagerFilter() {
        ...
        managers.forEach(manager => {
            ...
            option.textContent = this.formatNameForPrivacy(manager);
            select.appendChild(option);
        });
    }
    ...
    populateRoleFilter() {
        ...
        // Use relationship determination logic to create role options
        const roles = [...new Set(this.data.map(item => this.determineRelationship(item.manager, item.functionName, item.mdpName)).filter(role => role))].sort();
    ...
    populateRotationFilter() {
        ...
        console.log('Available rotations in data:', rotations);
        console.log('Sample data items with rotations:', this.data.slice(0, 5).map(item => ({mdpName: item.mdpName, rotation: item.rotation, type: typeof item.rotation})));
    ...
    applyFilters() {
        ...
        this.filteredData = this.data.filter(item => {
            ...
            if (this.filters.role && this.determineRelationship(item.manager, item.functionName, item.mdpName) !== this.filters.role) return false;
            
            // Debug rotation filtering - fix the comparison
            if (this.filters.rotation) {
                // Convert both to numbers for comparison
                const itemRotation = parseInt(item.rotation);
                const filterRotation = parseInt(this.filters.rotation);
                console.log(`Rotation filter: item.rotation=${itemRotation}, filterRotation=${filterRotation}`);
                if (itemRotation !== filterRotation) {
                    console.log(`Filtered out item with rotation ${itemRotation} (doesn't match filter ${filterRotation})`);
                    return false;
                }
            }
        ...
        console.log(`Filtered data: ${this.filteredData.length} items out of ${this.data.length} total`);
        if (this.filters.rotation) {
            const rotationData = this.data.filter(item => parseInt(item.rotation) === parseInt(this.filters.rotation));
            console.log(`Rotation ${this.filters.rotation} specific data:`, rotationData.map(item => ({ name: item.mdpName, rotation: item.rotation })));
        }
    ...
    getCohortAverages(rows) {
        console.log('getCohortAverages called with rows:', rows.length);
        if (!rows.length) {
            console.log('No rows found, returning empty averages');
            return { overall: 0, byArea: {}, byRotation: {} };
        }
    ...
    getTopPerformer(rows) {
        console.log('getTopPerformer called with rows:', rows.length);
        if (!rows.length) {
            console.log('No rows found, returning empty performer');
            return { mdpId: '', mdpName: '', score: 0, isTied: false };
        }
        
        // Group all responses by MDP name to calculate averages
        const mdpResponses = {};
        rows.forEach(item => {
            ...
            mdpResponses[key].push(item);
        });
        
        // Calculate average overall score for each MDP across all their responses
        const performers = [];
        Object.keys(mdpResponses).forEach(mdpName => {
            const responses = mdpResponses[mdpName];
            const totalScore = responses.reduce((sum, response) => sum + (response.overall || 0), 0);
            const averageScore = totalScore / responses.length;
            
            // Use the most recent response for other details (mdpId, etc.)
            const mostRecent = responses.reduce((latest, current) => 
                new Date(current.submittedAt) > new Date(latest.submittedAt) ? current : latest
            );
            
            performers.push({
                mdpId: mostRecent.mdpId || mostRecent.id || mdpName,
                mdpName: mdpName,
        ...
        console.log('Performers with averages:', performers);
        
        if (performers.length === 0) {
            console.log('No performers found after calculations');
            return { mdpId: '', mdpName: '', score: 0, isTied: false };
        }
        
        // Find the highest average score
        const maxScore = Math.max(...performers.map(p => p.averageScore));
        console.log('Max average score found:', maxScore);
        
        // Find all performers with the max average score
        const topPerformers = performers.filter(p => p.averageScore === maxScore);
    ...
    getAreaExtrema(byArea) {
        ...
        console.log('getAreaExtrema called with byArea:', byArea, 'areas:', areas);
        if (!areas.length) {
            console.log('No areas found, returning empty extrema');
            return { strengthArea: 'N/A', devArea: 'N/A' };
        }
        ...
        console.log('Strength area:', strengthArea, 'Dev area:', devArea);
    ...
    updateChartsOnly() {
        // Updates only data-driven content without resetting view state
        this.renderDynamicSummary();
        this.renderTiles();
        this.renderCharts();
        // Skip renderTable() to preserve table state (sorting, pagination, etc.)
    }

    async refresh() {
        // Manual refresh that preserves current state
        ...
        try {
            ...
            this.applyFilters();
            this.render(); // Full render for manual refresh is OK
            this.updateLastUpdated();
    ...
    renderTiles() {
        ...
        try {
            switch (this.viewMode) {
                case 'cohort':
                    console.log('Rendering cohort tiles...');
            ...
            grid.innerHTML = `<div style="grid-column: 1 / -1; text-align: center; padding: 2rem;">Error rendering tiles: ${error.message}</div>`;
        }
    }

    renderCohortTiles() {
        console.log('renderCohortTiles called, filteredData length:', this.filteredData.length);
        console.log('Sample filtered data:', this.filteredData.slice(0, 3));
        
        if (!this.filteredData.length) {
            console.log('No filtered data available for cohort tiles');
            return `<div class="tile" style="grid-column: 1 / -1; text-align: center; padding: 2rem;">
                <div class="tile-header"><div class="tile-title">No Data Available</div></div>
                <div class="tile-value">📊</div>
                <div class="tile-subtitle">No survey responses found</div>
            </div>`;
        }
        
        const averages = this.getCohortAverages(this.filteredData);
        console.log('Cohort averages:', averages);
        ...
        console.log('MDP count:', mdpCount);
    ...
    renderIndividualTiles() {
        console.log('renderIndividualTiles called, selectedIndividualMdp:', this.selectedIndividualMdp);
        console.log('Available filtered data:', this.filteredData.length, 'items');
        console.log('Looking for MDP with name:', this.selectedIndividualMdp);
        
        const mdpData = this.filteredData.filter(item => item.mdpName === this.selectedIndividualMdp);
        console.log('Found MDP data:', mdpData.length, 'records');
        
        if (!mdpData.length) {
            console.log('No data found for MDP:', this.selectedIndividualMdp);
            console.log('Available MDP names:', [...new Set(this.filteredData.map(item => item.mdpName))]);
            return `
                <div class="tile">
                    <div class="tile-header">
    ...
    createRotationChart() {
        ...
        if (!ctx) {
            console.log('rotationChart canvas not found');
            return;
        }

        const averages = this.getCohortAverages(this.filteredData);
        console.log('Rotation chart averages:', averages);
        console.log('Rotation data:', averages.byRotation);
        ...
        console.log('Rotation chart - rotations:', rotations);
        console.log('Rotation chart - data:', data);
    ...
    getCohortTableHeaders() {
        return `
            <tr>
                ...
                <th onclick="dashboard.sortTable('rotationCount')">Responses</th>
    ...
    getCohortTableRows() {
        ...
        return mdpSummaries.map((summary, index) => {
            // Get unique evaluators with their relationship roles for this MDP
            const evaluatorInfo = summary.rotations.map(r => ({
                name: r.manager,
                relationship: this.determineRelationship(r.manager, r.functionName, summary.mdpName)
            }));
            
            // Remove duplicates based on manager name and create display text
            const uniqueEvaluators = evaluatorInfo.reduce((acc, curr) => {
            ...
            const evaluatorText = uniqueEvaluators.map(e => 
                `${this.formatNameForPrivacy(e.name)} (${e.relationship})`
            ).join(', ');
    ...
    getIndividualTableRows() {
        ...
        // Group by rotation number only (not by function)  
        const rotationGroups = {};
        mdpData.forEach(item => {
            const key = String(item.rotation); // Ensure key is string for consistency
            if (!rotationGroups[key]) {
                rotationGroups[key] = {
            ...
            rotationGroups[key].submissions.push(item);
            rotationGroups[key].functions.add(item.functionName);
        });

        // Sort rotation groups by rotation number (1, 2, 3, 4)
        const sortedGroups = Object.values(rotationGroups).sort((a, b) => a.rotation - b.rotation);

        return sortedGroups.map(group => {
            // Calculate average scores across ALL submissions in this rotation (all functions)
            const submissions = group.submissions;
            const avgScores = {
                jobKnowledge: submissions.reduce((sum, s) => sum + s.jobKnowledge, 0) / submissions.length,
                qualityOfWork: submissions.reduce((sum, s) => sum + s.qualityOfWork, 0) / submissions.length,
                communication: submissions.reduce((sum, s) => sum + s.communication, 0) / submissions.length,
                initiative: submissions.reduce((sum, s) => sum + s.initiative, 0) / submissions.length,
                overall: submissions.reduce((sum, s) => sum + s.overall, 0) / submissions.length
            };

            // List of functions in this rotation
            const functionsList = Array.from(group.functions).join(', ');
            
            // Get unique evaluators with their relationship roles for this rotation
            const evaluatorInfo = submissions.map(s => ({
                name: s.manager,
                relationship: this.determineRelationship(s.manager, s.functionName, this.selectedIndividualMdp)
            }));
            
            // Remove duplicates and create display text with relationships
            const uniqueEvaluators = evaluatorInfo.reduce((acc, curr) => {
                const existing = acc.find(e => e.name === curr.name);
                if (!existing) {
                    acc.push(curr);
            ...
            return `
                <tr class="expandable-row" onclick="dashboard.expandRotationRow(${group.rotation}, this)">
                    <td>
                        <strong>Rotation ${group.rotation}</strong>
                        <br><small style="color: #666; font-size: 0.85em;">${functionsList}</small>
                    </td>
                    <td><span class="score-badge ${this.getScoreClass(avgScores.jobKnowledge)}">${avgScores.jobKnowledge.toFixed(1)}</span></td>
                    <td><span class="score-badge ${this.getScoreClass(avgScores.qualityOfWork)}">${avgScores.qualityOfWork.toFixed(1)}</span></td>
                    <td><span class="score-badge ${this.getScoreClass(avgScores.communication)}">${avgScores.communication.toFixed(1)}</span></td>
                    <td><span class="score-badge ${this.getScoreClass(avgScores.initiative)}">${avgScores.initiative.toFixed(1)}</span></td>
                    <td><strong><span class="score-badge ${this.getScoreClass(avgScores.overall)}">${avgScores.overall.toFixed(2)}</span></strong></td>
                </tr>
            `;
        }).join('');
    }
    ...
    expandRow(mdpName, rowElement) {
        ...
        // Sort by rotation number to ensure proper order (1, 2, 3, 4)
        const sortedMdpData = mdpData.sort((a, b) => a.rotation - b.rotation);
            ...
            <td colspan="5">
                <div style="padding: 1rem;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-top: 1rem;">
                        ${sortedMdpData.map(item => `
                            <div style="background: #f8f9fa; padding: 1rem; border-radius: 0.5rem;">
                                <strong>Rotation ${item.rotation} - ${item.functionName}</strong><br>
                                <small style="color: #666;">Evaluator: ${this.formatNameForPrivacy(item.manager)} (${this.determineRelationship(item.manager, item.functionName, item.mdpName)})</small><br><br>
    ...
    expandRotationRow(rotationNumber, rowElement) {
        // Toggle expanded content showing submission breakdown for individual view
        const existingExpanded = rowElement.nextElementSibling;
        if (existingExpanded && existingExpanded.classList.contains('expanded-content')) {
            existingExpanded.remove();
            return;
        ...
        const mdpData = this.filteredData.filter(item => 
            item.mdpName === this.selectedIndividualMdp && item.rotation === rotationNumber
        );
        ...
        expandedRow.innerHTML = `
            <td colspan="6">
                <div style="padding: 1rem;">
                    <h4>Rotation ${rotationNumber} Breakdown for ${this.formatNameForPrivacy(this.selectedIndividualMdp)}</h4>
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-top: 1rem;">
                        ${mdpData.map(item => `
                            <div style="background: #f8f9fa; padding: 1rem; border-radius: 0.5rem;">
                                <strong>Rotation ${item.rotation} - ${item.functionName}</strong><br>
                                <small style="color: #666;">Evaluator: ${this.formatNameForPrivacy(item.manager)} (${this.determineRelationship(item.manager, item.functionName, item.mdpName)})</small><br><br>
                                Job Knowledge: <span class="score-badge ${this.getScoreClass(item.scores['Job Knowledge'])}">${item.scores['Job Knowledge'].toFixed(1)}</span><br>
                                Quality of Work: <span class="score-badge ${this.getScoreClass(item.scores['Quality of Work'])}">${item.scores['Quality of Work'].toFixed(1)}</span><br>
                                Communication Skills & Teamwork: <span class="score-badge ${this.getScoreClass(item.scores['Communication Skills & Teamwork'])}">${item.scores['Communication Skills & Teamwork'].toFixed(1)}</span><br>
                                Initiative & Productivity: <span class="score-badge ${this.getScoreClass(item.scores['Initiative & Productivity'])}">${item.scores['Initiative & Productivity'].toFixed(1)}</span><br>
                                <br><strong>Overall: <span class="score-badge ${this.getScoreClass(item.overall)}">${item.overall.toFixed(2)}</span></strong>
                            </div>
                        `).join('')}
                    </div>
    ...
    determineRelationship(evaluatorName, evaluatorFunction, mdpName) {
        // Determine relationship type: Manager, Mentor, or Rotation Partner
        // Based on evaluator name and function patterns
        
        if (!evaluatorName || !evaluatorFunction) {
            return 'Manager'; // Default fallback
        }
        
        const evalName = evaluatorName.toLowerCase();
        const evalFunc = evaluatorFunction.toLowerCase();
        
        // Check for mentor indicators
        if (evalName.includes('mentor') || evalFunc.includes('mentor')) {
            return 'Mentor';
        }
        
        // Check for rotation partner indicators (peer level positions)
        if (evalFunc.includes('associate') || evalFunc.includes('analyst') || 
            evalFunc.includes('specialist') || evalFunc.includes('coordinator') ||
            evalName.includes('peer') || evalName.includes('partner')) {
            return 'Rotation Partner';
        }
        
        // Everything else defaults to Manager (most common case)
        return 'Manager';
    }
    ...
    clearFilters() {
        console.log('Clearing all filters and resetting to default state...');
        ...
        // Reset view mode to default (cohort)
        this.viewMode = 'cohort';
        
        // Reset selected MDPs for comparison and individual views
        this.selectedMdpA = '';
        this.selectedMdpB = '';
        this.selectedIndividualMdp = '';
        
        // Reset the filter UI elements
        ...
        // Reset the mode selector to cohort view
        const modeSelect = document.getElementById('modeSelect');
        if (modeSelect) {
            modeSelect.value = 'cohort';
        }
        
        // Reset the mode badge
        const modeBadge = document.getElementById('modeBadge');
        if (modeBadge) {
            modeBadge.textContent = 'Cohort';
        ...
        // Hide comparison selectors if they're visible
        const comparisonControls = document.getElementById('comparisonControls');
        if (comparisonControls) {
            comparisonControls.style.display = 'none';
        }
        
        // Re-populate filters with fresh data
        this.populateFilters();
        
        // Re-render the dashboard with cleared filters and default view
        this.render();
        
        // Save the reset state
        this.saveUserPreferences();
        
        console.log('All filters cleared, returned to cohort view');
    }
