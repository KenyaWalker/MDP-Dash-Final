...
// Check what evaluations already exist for the selected MDP and provide guidance
async function checkNextRotation() {
    ...
    try {
        ...
        // Show existing submissions info
        const rotationInfo = document.getElementById('rotationInfo');
        if (rotationInfo) {
            let infoText = `Existing evaluations for ${formatNameForPrivacy(mdpName)}:`;
            
            if (existingSurveys.length === 0) {
                infoText = `No previous evaluations found for ${formatNameForPrivacy(mdpName)}`;
            } else {
                // Group by rotation for display
                const byRotation = {};
                existingSurveys.forEach(survey => {
                    const rot = survey.rotation || 'Unknown';
                    if (!byRotation[rot]) byRotation[rot] = [];
                    // Only show role if it exists and is not empty
                    if (survey.evaluatorRole && survey.evaluatorRole.trim()) {
                        byRotation[rot].push(`${survey.functionName} (${survey.evaluatorRole})`);
                    } else {
                        byRotation[rot].push(survey.functionName);
                    }
                });
                
                Object.keys(byRotation).sort().forEach(rotation => {
                    infoText += `\nRotation ${rotation}: ${byRotation[rotation].join(', ')}`;
                });
            }
        ...
        // No longer disable function dropdown - allow multiple evaluations of same function by different roles
...
async function submitSurvey(event) {
    ...
    try {
        ...
        const rotationNumber = formData.get('rotationNumber');
        ...
        if (!rotationNumber) {
            throw new Error('Please select a rotation number');
        }
        ...
        const surveyData = {
            ...
            manager: formData.get('manager') || 'Anonymous',
            evaluatorRole: evaluatorRole,
            rotation: parseInt(rotationNumber),
            responses: responses,
            ...
            overall: overall,
            emailResponse: false, // Email functionality removed
            respondentEmail: null  // Email functionality removed
        };
        ...
        // Store survey data in localStorage for PDF generation
        localStorage.setItem('survey_mdpName', surveyData.mdpName);
        localStorage.setItem('survey_functionName', surveyData.functionName);
        localStorage.setItem('survey_manager', surveyData.manager);
        localStorage.setItem('survey_rotation', surveyData.rotation);
        localStorage.setItem('survey_submittedAt', surveyData.submittedAt);
        
        // Store individual responses
        Object.keys(surveyData.responses).forEach(key => {
            localStorage.setItem(`survey_${key}`, surveyData.responses[key]);
        });
...
// Email functionality removed for cleaner UI
